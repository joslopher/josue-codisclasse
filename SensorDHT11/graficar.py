...
def grafic(temperatura,humitat):   
    ...
    rects1 = ax1.bar(ind, dades, width, color='g')	# Dades dels diagrames: CoordenadesX, Valors de les variables, Amplada i Color.
    ax1.set_ylabel('Temperatura')			# Llegenda de eix Y.
    ax1.set_title('Temperatures')			# Títol del gràfic
    ax1.set_xticks(ind)					# Marques sota de cada diagrama
    ax1.yaxis.set_major_locator(plt.NullLocator())
    ax1.set_xticklabels(('10 muestras'))		# Text sota de cada diagrama en aquest cas 
    ax1.set_yticklabels([])
    ...
    rects2 = ax2.bar(ind, dades2, width, color='r')	# Dades dels diagrames: CoordenadesX, Valors de les variables, Amplada i Color.
    ax2.set_ylabel('Humitat')				# Llegenda de eix Y.
    ax2.set_title('Humitats')				# Títol del gràfic
    ax2.set_xticks(ind)					# Marques sota de cada diagrama
    ax2.yaxis.set_major_locator(plt.NullLocator())
    ax2.set_xticklabels(('10 muestras'))		# Text sota de cada diagrama.
    ax2.set_yticklabels([])
